...
import { installRemoteRuntime, installRemoteWorkspace } from './workspace.mjs';
...
for (const tenant of tenants) {
  ...
  if (process.env.REMOTE_CONNECT_RUNTIME_HOOKS === '1') installRemoteRuntime(path.join(root, 'tenants', tenant.id));
  count++;
}
