import { assertSafePath, updateConfig, updateBlock } from '../owner-state/index.mjs';
...
export function installPhoneHandoff(dir, tenant) {
  for (const relative of ['config/openclaw.json', 'workspace/AGENTS.md', 'workspace/phone/account.json', 'workspace/phone/session.mjs', 'workspace/skills/phone/SKILL.md']) assertSafePath(path.join(dir, relative));
  ...
  const resultingConfig = updateConfig(dir, 'phone-plugin', config => {
  ...
  if (owner || prior) {
    ...
    if (owner) {
      config.plugins.entries[pluginId] = { enabled: true, ...prior, hooks: { allowConversationAccess: true, allowPromptInjection: true, ...prior?.hooks }, config: { owner, ...prior?.config } };
  ...
  if (existsSync(agents)) {
    ...
    const block = next.match(/<!-- managed-phone-handoff:start -->[\s\S]*?<!-- managed-phone-handoff:end -->\n?/)?.[0] || '';
    updateBlock(dir, 'managed-phone-handoff', agents, block);
  }
  const changed = JSON.stringify(resultingConfig) !== JSON.stringify(JSON.parse(raw));
