...
for (let attempt = 0; attempt < 90 && pending.size; attempt++) {
  await Promise.all([...pending].map(async tenant => {
    ...
    if (!active?.includes(marker)) return;
    try {
      const result = await new Promise((resolve, reject) => {
        const child = execFile('docker', ['exec', '-i', container, 'node', '--input-type=module'], { timeout: 15000, maxBuffer: 65536 }, (error, stdout) => error ? reject(error) : resolve(stdout));
        child.stdin.end(probe);
      });
      console.log(`${tenant}: ${result.trim()}`); pending.delete(tenant);
    } catch { /* Allow the first authenticated history sync to finish. */ }
  }));
  if (pending.size) await new Promise(resolve => setTimeout(resolve, 5000));
}
if (pending.size) throw new Error('Gateways have not activated and synchronized the new handoff: ' + [...pending].join(', '));
