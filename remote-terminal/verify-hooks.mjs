...
const dir = mkdtempSync(path.join(tmpdir(), 'terminal-hooks-runtime-'));
const requests = [];
let gateway;
...
try {
  ...
  mkdirSync(path.join(workspace, 'remote-terminal'), { recursive: true });
  writeFileSync(path.join(workspace, 'remote-terminal/session.mjs'), `import assert from 'node:assert/strict'; assert.equal(process.argv[2], 'create'); console.log(JSON.stringify({url:'https://8examples.com/remote-terminal/00000000-0000-4000-8000-000000000000',code:'000123',expiresAt:'2099-01-01T00:00:00Z'}));`);
  ...
  writeFileSync(config, JSON.stringify({
    ...
    plugins: { allow: ['managed-remote-terminal'], load: { paths: [process.argv[2]] }, entries: { 'managed-remote-terminal': { enabled: true, hooks: { allowConversationAccess: true, allowPromptInjection: true } } } },
  }));
  ...
  const { stdout } = await exec('openclaw', ['agent', '--agent', 'main', '--session-key', 'agent:main:telegram:direct:smoke', '--message', "Please open a remote terminal", '--timeout', '60', '--json'], {
    env, timeout: 90000, maxBuffer: 2 * 1024 * 1024,
  });
  assert.ok(stdout.includes('https://8examples.com/remote-terminal/00000000-0000-4000-8000-000000000000'));
  ...
  assert.ok(requests.every(body => body.includes('REMOTE TERMINAL is installed and enabled')), 'The provider must receive the runtime system context');
  console.log('PASS: native OpenClaw creates a terminal handoff directly, preserves the six-digit code, and injects the managed capability into normal model requests.');
} finally {
