...
test('real PTY supports sizing, Unicode, no-echo password prompts, Ctrl+C, cursor replay and cleanup', async t => {
  ...
  const text = () => terminal.read().chunks.map(c => Buffer.from(c.data, 'base64').toString()).join('');
  ...
  await until(() => text().includes('Secret prompt: '));
  // Wait for bash to actually execute read, after the echoed command line.
  ...
  await input('sleep 60\r'); await new Promise(resolve => setTimeout(resolve, 80)); await input('\x03');
  await input("printf 'AFTER_INTERRUPT\\n'; printf 'saved' > terminal-proof.txt\r");
  await until(() => existsSync(path.join(dir, 'terminal-proof.txt')));
  assert.match(text(), /AFTER_INTERRUPT/);
