...
/// Other ways to reach an 8Examples AI Assistant: Telegram and Amazon Alexa.
            ...
            List {
                Section("Reach your assistant from…") {
                    ...
                    NavigationLink(value: "alexa") {
                        Label("Amazon Alexa", systemImage: "waveform.circle.fill")
                    }
                }
                Section("Also") {
    ...
    var body: some View {
        List {
            ...
            Section("Set up a Telegram bot for your assistant") {
                ...
                Step(3, "Send **/newbot**. Give it a name (e.g. “Ana's Assistant”) and a username ending in **bot** (e.g. `anas_assistant_bot`).")
            ...
            Section("Good to know") {
                Text("Only you can talk to your bot: your assistant remembers the first Telegram account that says hi and ignores strangers. Anything you tell it on Telegram, in this app, or on the phone reaches the same assistant with the same memory.")
                    .font(.footnote).foregroundStyle(.secondary)
                Button {
        ...
        List {
            Section {
                VStack(alignment: .leading, spacing: 6) {
                    Text("“Alexa, ask 8Examples AI Assistant what's on my calendar today.”")
                        .font(.headline)
                    Text("The **8Examples AI Assistant** skill lives in the Alexa Skills store under **Productivity**. Enable it once, link it to this account, and every Echo in the house can reach your assistant.")
                        .font(.subheadline).foregroundStyle(.secondary)
                }
                .padding(.vertical, 4)
                Button {
                    openURL(AppConfig.alexaSkillStoreURL)
                } label: { Label("Open 8Examples AI Assistant in the Alexa Skills store", systemImage: "arrow.up.right.square") }
                Button {
            ...
            Section("Pair it (once)") {
                ...
                Step(2, "Tap **More → Skills & Games**, search **8Examples AI Assistant**, or browse **Categories → Productivity**.")
                Step(3, "Tap **Enable to Use**.")
                Step(4, "When asked to **Link Account**, sign in with the same username and password as this app (\(model.me?.username ?? "your 8Examples login")).")
                Step(5, "Done — the skill is now paired with your assistant\(model.claws.count > 1 ? "s" : "").")
            }

            Section("Ask for it on any Alexa device") {
                Step(1, "Say **“Alexa, open 8Examples AI Assistant.”** — then talk normally.")
                Step(2, "Or in one breath: **“Alexa, ask 8Examples AI Assistant to call the dentist and book a cleaning.”**")
                Step(3, "**“Alexa, tell 8Examples AI Assistant I'm leaving now.”** — handy with location sharing on.")
                Step(4, "**“Alexa, ask 8Examples AI Assistant what's new.”** — your assistant reads out anything waiting for you.")
            }

            Section("Troubleshooting") {
                Text("If Alexa says the skill isn't linked, open the Alexa app → More → Skills & Games → Your Skills → 8Examples AI Assistant → Settings → Link Account and sign in again.")
                    .font(.footnote).foregroundStyle(.secondary)
            }
        }
